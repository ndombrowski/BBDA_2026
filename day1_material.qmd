...
```{bash}
# Install busco and transdecoder
mamba create \
...
# was not updated for a new folder layout, so the new `TransDecoder` wrapper
# never gets linked into bin/. The script itself is still installed under opt/,
# so link it in manually:
ln -s <your_path>/transcriptomics_bbda/opt/transdecoder/TransDecoder \
      <your_path>/transcriptomics_bbda/bin/TransDecoder

# Install trintity separately since it has many dependencies
mamba create \
...
3.  Was an unstranded or stranded sequencing protocol used? If stranded, which library type (RF or FR) was used?
...
In transcriptomics it is important to know what library type was used to generate the data, as this affects downstream steps. You may encounter the following terminology when exploring the manuals of different software tools:

<center>![](img/strands.png){width="70%"}</center>

How to read the figure: the purple bar is the transcript. Blue arrows have the same sequence as the transcript (sense), green arrows have the opposite sequence (antisense). R1 (bold) is read 1 and R2 is read 2 of a read pair, and the dashed line is the part of the fragment that is not sequenced. In a direct stranded library, read 1 runs in the same direction as the transcript. In a reverse stranded library, read 1 runs opposite to it. In an unstranded library both happen, so you cannot tell which strand a read came from.

\*During library prep the RNA is first copied into first-strand cDNA (antisense sequence), followed by second-strand cDNA (sense sequence). The name tells you which of the two has the same sequence as read 1.
...
3.  We work with reverse stranded sequencing data. Two parts of the methods section tell you more about this:
    -   The authors used the Illumina TruSeq® Stranded mRNA protocol to prepare the sequencing libraries. If you search for more information about this kit online, you will also find the direction of the strand. According to [Illumina documentation](https://knowledge.illumina.com/library-preparation/rna-library-prep/library-preparation-rna-library-prep-reference_material-list/000002238), Read 1 is antisense to the transcript, and Read 2 to the sense strand, meaning a reverse stranded protocol was used.
...
- On the average quality score: seqkit's AvgQual can look much lower than the FastQC plots suggest. That is expected, the two tools average differently and FastQC's method tends to overestimate. For a reliable read on overall quality, use the Q20(%)/Q30(%) columns from seqkit instead of comparing the two average numbers directly.
- On the adapter content: if you look at the FastQC Adapter Content plot, the adapter shows up as a slowly rising line (Illumina Universal Adapter) that only appears in the last quarter or so of the read, reaching about 1-2% by the final base. This stays low enough to still count as a "pass" in FastQC's own scoring, but it is worth removing before assembly.
- On Per base sequence content: you will likely see this module marked as a warning or fail too, with the four base lines (A/C/G/T) jumping around a lot over roughly the first 10-13 bases before settling into flat, parallel lines for the rest of the read, for example:

    <center>![](img/fastqc_per_base_content_example.png){width="70%"}</center>

    This is a known artifact of the random hexamer priming used to make the cDNA during RNA-seq library prep, which introduces a slight base composition bias at the very start of the read. It is expected and not something you need to trim or otherwise correct for.
- On Per sequence GC content: you will likely see this module marked as a warning or fail, with a curve (red) that is narrower than, and sometimes has a small second bump compared to, the smooth theoretical curve (blue), for example:

    <center>![](img/fastqc_gc_content_example.png){width="70%"}</center>

    A shape like this is common in RNA-seq, since only expressed genes are sequenced instead of the whole genome, and a handful of highly expressed genes can dominate the read counts and shift or bump the curve. But a similar-looking bump can also be caused by contamination (e.g. reads from another organism), and the plot on its own can't tell the two apart. To dig further, you could:

    - check the "Overrepresented sequences" module in the same report, which lists the exact sequences behind any spike
    - compare the sample's overall %GC (top of the report) to a published value for the organism if one is available; a %GC far outside that range points more towards contamination, while one that's close is reassuring, though it doesn't rule out a small contaminating fraction. For this species/dataset we don't have a reference %GC to compare against.
- On Sequence Duplication Levels: this module is often a warning or fail in RNA-seq too. Unlike genomic DNA, where you would expect fairly even coverage, RNA-seq reads come from genes expressed at very different levels; a small number of highly expressed genes get sequenced many times over, which FastQC reports as duplicates, for example:

    <center>![](img/fastqc_seq_duplication_example.png){width="70%"}</center>

    This is expected, and is one of the reasons we don't remove duplicate reads before assembly or quantification in RNA-seq: doing so would remove real signal from highly expressed genes, not just technical artifacts.
:::
:::
...
fastp -i ${DATA}/${SAMPLE}_1.fastq.gz \
    ...
    --cut_window_size 3 --cut_mean_quality 25 --length_required 50 \
    --thread 4 
'''
...
# We now work with x reads (fill this in yourself)
...
::: {.callout-tip title="Answer" collapse="true" appearance="simple"}
The cleaning worked. The quality is slightly better and the adapter sequences are gone. We still work with \~97% of the data (ca. 58,411,986 / 60,000,000 reads are maintained), which is in a very good range.
:::
:::
...
#        - Ensure that you set the library type you found in Question 3 ("The data")
