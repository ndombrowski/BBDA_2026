...
In a previous tutorial you worked on a **reference-guided assembly**, where sequencing reads are aligned to a known reference genome to reconstruct transcripts and identify differentially expressed genes (DEGs). In this tutorial, you'll be introduced to **de novo transcriptome assembly**, or reference-free assembly, where transcripts are assembled using only the information contained in the reads themselves.
...
4.  Based on the [sample information on ENA](https://www.ebi.ac.uk/ena/browser/view/PRJEB12372) generate a mapping file, i.e. a file that links the sample IDs with more descriptive information:
    -   Use the Run Accessions from `/zfs/omics/projects/education/BBDA_transcriptomics/denovo/data` (e.g., ERR1211089) and find the corresponding treatment by exploring the information on ENA that is stored under the Sample Accession. 
...
Although our FastQC report showed good quality, this is uncommon in real datasets. Therefore, we will now clean the sequences using `fastp` [@Chen2018]. To learn about the different options of this tool, you can go [here](https://github.com/OpenGene/fastp).
...
Since fastp will run for a bit longer and we want to run it on multiple samples, we will now write our first SLURM job script. For this, we could use a [for-loop](https://scienceparkstudygroup.github.io/ibed-bioinformatics-page/source/core_tools/bash-for-loops.html), which would run all 6 fastp steps one after the other. You can also use [SLURM arrays](https://blog.ronin.cloud/slurm-job-arrays/) to parallelize these steps and, for example, run 2 fastp steps at the same time. Because you’ve likely already worked with for-loops, we’ll show you an example of using a SLURM array instead.
...
# Run fastp
# Estimated run time: < 5 min
# Hints:    - Use the DATA and SAMPLE variables to build file names
#           - Trim reads where quality drops below 25 in a 3 bp window
#           - Discard reads shorter than 50 bp
#           - Save paired outputs in two separate files
